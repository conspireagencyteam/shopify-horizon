...
export class ProductCard extends Component {
  ...
  #handleVariantUpdate = (event) => {
    ...
    // Update the form's variant ID input since stopPropagation() prevents
    // the VariantUpdateEvent from reaching the product-form-component
    let variantInput = this.refs.quickAdd?.querySelector('input[name="id"]');
    if (!variantInput) {
      variantInput = this.querySelector('input[name="id"]');
    }

    if (variantInput && event.detail.resource?.id) {
      variantInput.value = event.detail.resource.id;
    }
  ...
  #updateOverflowList() {
    ...
    const overflowList = this.querySelector(
      'swatches-variant-picker-component overflow-list, non-swatch-variant-picker-component overflow-list'
    );
  ...
  get variantPicker() {
    return (
      this.querySelector('swatches-variant-picker-component') ||
      this.querySelector('non-swatch-variant-picker-component')
    );
  }
  /** @type {number | null} */
...
class SwatchesVariantPickerComponent extends VariantPicker {
  ...
  /**
   * Override updateVariantPicker to preserve overflow list expanded state
   * @param {Document} newHtml - The new HTML.
   */
  updateVariantPicker(newHtml) {
    // Preserve overflow list "show all" state before morphing
    const oldOverflowList = this.querySelector('overflow-list');
    const wasShowingAll = oldOverflowList?.hasAttribute('disabled');

    // Call parent method
    const result = super.updateVariantPicker(newHtml);

    // Restore "show all" state after morphing
    if (wasShowingAll) {
      const newOverflowList = this.querySelector('overflow-list');
      if (newOverflowList) {
        newOverflowList.setAttribute('disabled', 'true');
        const placeholder = newOverflowList.shadowRoot?.querySelector('[part="placeholder"]');
...
/**
 * A custom element that displays a variant picker with non swatches.
 *
 * @typedef {object} NonSwatchesRefs
 * @property {HTMLElement} overflowList
 *
 * @extends {VariantPicker<NonSwatchesRefs>}
 */
class NonSwatchVariantPickerComponent extends VariantPicker {
  connectedCallback() {
    super.connectedCallback();
  ...
  /**
   * Override the variantChanged method to track the selected variant ID.
   * This is necessary to update the product card link URL with the correct variant parameter.
   * @param {Event} event - The variant change event.
   */
  variantChanged(event) {
    if (!(event.target instanceof HTMLElement)) return;
    ...
    // Store the variant ID to be applied to the URL after the variant update event
    if (selectedOption?.dataset.variantId) {
      this.pendingVariantId = selectedOption.dataset.variantId;
    }

    // Call the parent's variantChanged method
    super.variantChanged(event);
  }
  ...
  updateVariantPicker(newHtml) {
    ...
    if (wasShowingAll) {
      ...
      if (newOverflowList) {
        newOverflowList.setAttribute('disabled', 'true');
        const placeholder = newOverflowList.shadowRoot?.querySelector(
          '[part="placeholder"]'
        );
        if (placeholder instanceof HTMLElement) {
          placeholder.style.setProperty('width', '0');
  ...
  /**
   * Shows all non swatches.
   * @param {Event} [event] - The event that triggered the show all non swatches.
   */
  showAllNonSwatches(event) {
    event?.preventDefault();
...
if (!customElements.get('non-swatch-variant-picker-component')) {
  customElements.define(
    'non-swatch-variant-picker-component',
    NonSwatchVariantPickerComponent
  );
}
